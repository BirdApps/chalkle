# encoding: UTF-8
class DiscussionMailer < BaseChalkleMailer

  default from: '"chalkle°" <learn@chalkle.com>' 

  def new_from_teacher(course_notice, chalkler)
    @notice = course_notice
    @chalkler = chalkler
    mail(to: @chalkler.email,  subject: I18n.t("email.chalkler.discussion.new_from_teacher.subject", from_name: @notice.chalkler.first_name,  course_name: @notice.course.name)) do |format| 
      format.text { render layout: 'standard_mailer' }
      format.html { render layout: 'standard_mailer' }
    end
  end

  def new_from_chalkler(course_notice, chalkler)
    @notice = course_notice
    @chalkler = chalkler
    mail(to: @chalkler.email,  subject: I18n.t("email.chalkler.discussion.new_from_teacher.subject", from_name: @notice.chalkler.first_name,  course_name: @notice.course.name)) do |format| 
      format.text { render layout: 'standard_mailer' }
      format.html { render layout: 'standard_mailer' }
    end
  end

end
