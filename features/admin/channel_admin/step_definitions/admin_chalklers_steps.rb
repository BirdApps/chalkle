...
Given(/^"(.*?)" is the teacher for the course "(.*?)"$/) do |teacher_name, course_name|
  teacher = Chalkler.find_by_name teacher_name
  course = FactoryGirl.create(:course, name: course_name, teacher_id: teacher.id, channel: teacher.channels.first)
end

Then(/^they should see the course "(.*?)"$/) do |name|
  page.should have_content(name)
end

Given(/^"(.*?)" attended the course "(.*?)"$/) do |student_name, course_name|
  student = Chalkler.find_by_name student_name
  course = FactoryGirl.create(:course, name: course_name, channel: student.channels.first)
  FactoryGirl.create(:booking, course_id: course.id, chalkler_id: student.id, status: 'yes')
end

Given(/^"(.*?)" did not attend the course "(.*?)"$/) do |student_name, course_name|
  student = Chalkler.find_by_name student_name
  course = FactoryGirl.create(:course, name: course_name, channel: student.channels.first)
  FactoryGirl.create(:booking, course_id: course.id, chalkler_id: student.id, status: 'no')
end

Then(/^they should not see the course "(.*?)"$/) do |name|
...
When(/^they edit "(.*?)'s" profile$/) do |arg1|
  chalkler = Chalkler.find_by_name arg1
  visit "/admin/chalklers/#{chalkler.id}/edit"
  ActionMailer::Base.deliveries = []
end

Then(/^they should not see the "(.*?)" form item$/) do |form_item|
  page.should_not have_content(form_item)
end
