Given /^there is an unreviewed lesson in the "(.*?)" channel$/ do |channel_name|
  ...
  lesson = FactoryGirl.create(:lesson, name: "Test Class", status: "Unreviewed", cost: 5, channel: channel)
end
...
Given /^there is a lesson in the "(.*?)" channel being processed$/ do |channel_name|
  ...
  Lesson.where(:name => "Test Class").destroy_all
  lesson = FactoryGirl.create(:lesson, name: "Test Class", status: "Processing", cost: 5, channel: channel)
end

Given /^there is an approved lesson in the "(.*?)" channel$/ do |channel_name|
  channel = Channel.where(name: channel_name).first_or_create!([name: channel_name, url_name: channel_name.downcase, email: "#{channel_name.downcase}@chalkle.com"], :as => :admin)
  Lesson.where(:name => "Test Class").destroy_all
  lesson = FactoryGirl.create(:lesson, name: "Test Class", status: "Approved", cost: 5, channel: channel)
end

Given /^there is an on-hold lesson in the "(.*?)" channel$/ do |channel_name|
  channel = Channel.where(name: channel_name).first_or_create!([name: channel_name, url_name: channel_name.downcase, email: "#{channel_name.downcase}@chalkle.com"], :as => :admin)
  Lesson.where(:name => "Test Class").destroy_all
  lesson = FactoryGirl.create(:lesson, name: "Test Class", status: "On-hold", cost: 5, channel: channel)
end

Given /^there is lesson in the "(.*?)" channel coming up this week$/ do |channel_name|
  channel = Channel.where(name: channel_name).first_or_create!([name: channel_name, url_name: channel_name.downcase, email: "#{channel_name.downcase}@chalkle.com"], :as => :admin)
  Lesson.where(:name => "Test Class").destroy_all
  lesson = FactoryGirl.create(:lesson, name: "Test Class", status: "Published", start_at: 2.days.from_now, cost: 5, channel: channel)
end
...
Given /^there is lesson in the "(.*?)" channel coming up this week with minimum attendee of "(.*?)"$/ do |channel_name, min_attendee|
  ...
  lesson = FactoryGirl.create(:lesson, name: "Test Class", teacher_id: teacher.id, start_at: 2.days.from_now, do_during_class: "Nothing much", teacher_cost: nil, venue_cost: 10, status: "Published", cost: 10, venue: "Town Hall", min_attendee: min_attendee, channel: channel)
end
...
Given(/^there is lesson in the "(.*?)" channel coming up this week with no teacher cost$/) do |name|
  ...
  lesson = FactoryGirl.create(:lesson, name: "Test class", teacher_id: teacher.id, start_at: 2.days.from_now, do_during_class: "Nothing much", teacher_cost: nil, venue_cost: 10, status: "Published", cost: 10, venue: "Town Hall", channel: channel)
