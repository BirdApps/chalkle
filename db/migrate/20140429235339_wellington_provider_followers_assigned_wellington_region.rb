class WellingtonProviderFollowersAssignedWellingtonRegion < ActiveRecord::Migration
  def up
    if wellington_region_id = Region.find_by_name("Wellington").try(:id)
      wellington_chalklers = Provider.where(name: "Wellington").map(&:chalklers).flatten!
      transaction do 
